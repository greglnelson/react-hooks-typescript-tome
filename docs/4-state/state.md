...
Unfortunately, it is extremely likely that you will encounter "merge" conflicts between your code and our code. These conflicts occur because we are providing edits to a file that you have also edited. If you are lucky, the conflicts will be automatically merged. However, it is just as likely that there will be merge conflicts that you must resolve. If you get a merge conflict or just are not familiar with resolving merge conflicts, watch this [excellent ~8 minute video](https://www.youtube.com/watch?v=HosPml1qkrg) that walks through what a merge conflict is, and how to use VSCode to resolve the conflicts in a single file. The VS Code editor can make choosing the appropriate updated code easier, though you still will have to merge the chosen change. Once you've resolved all the conflicts (potentially multiple files will each have merge conflicts), you can then add all your files and commit to finish your merge. You may want to read the rest of the instructions to understand what the final version of this branch should look like.

(If you want practice outside of the textbook on merge conflict and resolving them, see this [tutorial on merging conflicts](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/addressing-merge-conflicts/resolving-a-merge-conflict-using-the-command-line).

If your repository still has the `HtmlCss.test.tsx` and `text.test.tsx` files, then we give you special permission to remove those files from your new `solved-state` branch. You can also remove the old HTML that you added in your `App` that is no longer relevant to the new tasks we are working on, so that the screen is not crowded. Fortunately, none of that code is gone - it is all safely available in the other branches you made before, in case you ever need it again. This is the power of Version Control! (Do not misinterpret this to mean the general way to "fix" merge conflicts is to just delete files)
